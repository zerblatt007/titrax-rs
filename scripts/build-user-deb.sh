...
Binary resolution order when no path is given:
  1) target/release/titrax  (if --release)
  2) target/debug/titrax
  3) target/release/titrax
  4) cargo build (debug)
EOF
}
...
if [[ ! -f "${ROOT_DIR}/share/icons/hicolor/64x64/apps/titrax.png" ]]; then
    echo "error: missing icon file share/icons/hicolor/64x64/apps/titrax.png" >&2
    exit 1
fi

version=$(grep '^version' "${ROOT_DIR}/Cargo.toml" | head -1 | sed 's/version = "\(.*\)"/\1/')
arch=$(dpkg --print-architecture 2>/dev/null || echo all)
pkg_name="titrax-user"
deb_name="${pkg_name}_${version}_${arch}.deb"

build_root="$(mktemp -d /tmp/titrax-user-deb-XXXXXX)"
...
install -m 755 "${SOURCE_BIN}" "${build_root}/bin/titrax"
install -m 644 "${ROOT_DIR}/share/icons/hicolor/64x64/apps/titrax.png" \
    "${build_root}/share/icons/hicolor/64x64/apps/titrax.png"
cat > "${build_root}/share/applications/titrax.desktop" <<EOF
[Desktop Entry]
Type=Application
Name=TimeTracker
...
if [[ ${INSTALL_MODE} -eq 1 ]]; then
    ...
    echo "Binary: ${user_prefix}/bin/titrax"
    echo "Desktop: ${user_prefix}/share/applications/titrax.desktop"
fi
